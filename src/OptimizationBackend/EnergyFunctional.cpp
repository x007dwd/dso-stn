...
//这里accumulateAF_MT中的A对应accSSE_top_A，下边还有一个函数是accumulateLF_MT，对应的是accSSE_top_L。
// MT 指代的是是否需要多线程处理
// multiThreading,
// 所以可以看到addPointsInternal和addPoint的作用是一样的，不过或者少了双层循环。
//另外 AccumulatedTopHessianSSE 和
// AccumulatedSCHessianSSE之间存在那些不同，为什么需要设置两个文件（类）来完成他们？
...
// VecCf表示 CPARS 数量的float vector CPARS表示intrinsic parameters
void EnergyFunctional::resubstituteFPt(VecCf xc, Mat18f *xAd, int min, int max,
                                       Vec10 *stats, int tid) {
  ...
  for (int i = min; i < max; i++) {
    ...
    for (EFResidual *r : p->residualsAll) {
      if (!r->isLinearized || !r->isActive())
        continue;
      // isLinearized标志在fixLinearizationF s置1
      // 被函数linearizeAll_Reductor调用
...
double EnergyFunctional::calcLEnergyF_MT() {
  ...
  for (EFFrame *f : frames) {
    ...
    //
    E += f->delta_prior.cwiseProduct(f->prior)
             .cwiseProduct(hw)
             .cwiseProduct(Vec8::Ones() * 2 - hw)
             .dot(f->delta_prior);
  }
