...
void AccumulatedTopHessianSSE::stitchDouble(MatXX &H, VecX &b,
  ...
  for (int h = 0; h < nframes[tid]; h++)
    for (int t = 0; t < nframes[tid]; t++) {
      ...
      // adHost 使用来进行
      // Eigen Block
      // 函数使用的<>表示提取块大小，（）表示起始点。，使用noaliasd()函数会避免产生临时变量
  ...
  if (usePrior) {
    ...
    for (int h = 0; h < nframes[tid]; h++) {
      Vec8 hw = Vec8::Ones();
      if (fabs((double)(EF->frames[h]->delta_prior[6])) >
          setting_affineOptModeA_huberTH)
        hw[6] = 1 / fabs((double)(EF->frames[h]->delta_prior[6]));
      if (fabs((double)(EF->frames[h]->delta_prior[7])) >
          setting_affineOptModeB_huberTH)
        hw[7] = 1 / fabs((double)(EF->frames[h]->delta_prior[7]));

      H.diagonal().segment<8>(CPARS + h * 8) +=
          EF->frames[h]->prior.cwiseProduct(hw);
      b.segment<8>(CPARS + h * 8) +=
          EF->frames[h]
              ->prior.cwiseProduct(EF->frames[h]->delta_prior)
              .cwiseProduct(hw);
    }
  }
}
