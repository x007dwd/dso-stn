...
void FullSystem::setNewFrameEnergyTH() {
  ...
  newFrame->frameEnergyTH = nthElement * setting_frameEnergyTHFacMedian +
                            meanElement * setting_frameEnergyTHFacMean;
...
float FullSystem::optimize(int mnumOptIts) {
  ...
  // activeResiduals是关于的PointFrameResidual的指针的数组（向量）。
  int numPoints = 0;
  int numLRes = 0;
  for (FrameHessian *fh : frameHessians)
    for (PointHessian *ph : fh->pointHessians) {
  ...
  // VecX Dynamic Eigen Vector 在util/NumType.h中定义
...
std::vector<VecX> FullSystem::getNullspaces(
  ...
  // #define CPARS 4  在util/NumType.h中定义
