...
ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian *host_, float type,
  ...
  for (int idx = 0; idx < patternNum; idx++) {
    ...
    if (setting_killOverexposedMode == 1)
      ptc = getInterpolatedElement33OverAnd(host->dI, host->overexposedMap,
                                            u + dx, v + dy, wG[0], ptcOver);
    else
      ptc = getInterpolatedElement33OverOr(host->dI, host->overexposedMap,
                                           u + dx, v + dy, wG[0], ptcOver);
...
ImmaturePoint::traceOn(FrameHessian *frame, Mat33f hostToFrame_KRKi,
  ...
  //check, be slightly more relaxed by this factor.
  //	const float slackInterval = 0.8;			// if pixel-interval is
  //smaller than this, leave it be.
  //	const float minImprovementFactor = 2;		// if pixel-interval is
  //smaller than this, leave it be.
