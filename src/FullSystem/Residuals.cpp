  ...
  for (int idx = 0; idx < patternNum; idx++) {
    ...
    if (setting_killOverexposedMode == 0) {
      ...
      if (setting_killOverexposedMode == 1)
        hitColor = getInterpolatedElement33OverAnd(dIl, targetOverMap, Ku, Kv,
                                                   wG[0], hitOver);
      else
        hitColor = getInterpolatedElement33OverOr(dIl, targetOverMap, Ku, Kv,
                                                  wG[0], hitOver);

      residual = hitColor[0] - (float)(affLL[0] * color[idx] + affLL[1]);

      if (residual > 0 && refOver[idx])
        overexposedWeight = 0;
