...
Vec6 CoarseTracker::calcRes(int lvl, SE3 refToNew, AffLight aff_g2l,
  ...
  for (int i = 0; i < nl; i++) {
    ...
    if (setting_killOverexposedMode == 0) {
      ...
      hw = fabs(residual) < setting_huberTH ? 1
                                            : setting_huberTH / fabs(residual);
    } else {
      bool hitOver;
      bool refOver = lpc_over[i];
      if (setting_killOverexposedMode == 1)
        hitColor = getInterpolatedElement33OverAnd(dINewl, overexposedNewl, Ku,
                                                   Kv, wl, hitOver);
      else
        hitColor = getInterpolatedElement33OverOr(dINewl, overexposedNewl, Ku,
                                                  Kv, wl, hitOver);

      if (!std::isfinite(refColor) || !std::isfinite((float)hitColor[0]))
        continue;
      residual = hitColor[0] - (float)(affLL[0] * refColor + affLL[1]);
      hw = fabs(residual) < setting_huberTH ? 1
                                            : setting_huberTH / fabs(residual);
      if (residual > 0 && refOver) {
        if (debugPlot)
          resImage->setPixel4(lpc_u[i], lpc_v[i], Vec3b(255, 255, 0));
        continue;
      }
      if (residual < 0 && hitOver) {
        if (debugPlot)
          resImage->setPixel4(lpc_u[i], lpc_v[i], Vec3b(0, 255, 0));
        continue;
      }
    }
...
void CoarseTracker::debugPlotIDepthMap(std::string name, float *minID_pt,
                                       float *maxID_pt,
                                       IOWrap::Output3DWrapper *wrap) {
...
void CoarseDistanceMap::debugPlotDistMap(std::string name) {
  ...
  //	if(w[0] == 0) return;
  //	cv::Mat wrap = 0.8*cv::Mat(h[1], w[1], CV_64F,
  //const_cast<float*>(fwdWarpedIDDistFinal)) / (float)10;
