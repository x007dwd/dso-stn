...
Vec4 FullSystem::trackNewCoarse(FrameHessian *fh) {
  ...
  if (outputWrapper != 0)
    outputWrapper->pushLiveFrame(fh);
  ...
  for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
    ...
    {
      if (haveOneGood && i < 5 &&
          achievedRes[0] < lastCoarseRMSE[0] * setting_reTrackThreshold1)
        break;
      else if (haveOneGood && i >= 5 && i < 15 &&
               achievedRes[0] < lastCoarseRMSE[0] * setting_reTrackThreshold2)
        break;
      else if (haveOneGood && i >= 15 &&
               achievedRes[0] < lastCoarseRMSE[0] * setting_reTrackThreshold3)
        break;
    }
  }
...
void FullSystem::activatePointsMT() {
  ...
  for (FrameHessian *host : frameHessians) // go through all active frames
  {
    ...
    for (unsigned int i = 0; i < host->immaturePoints.size(); i += 1) {
      ...
      if (setting_activateAllOnMarg && (ph->host->flaggedForMarginalization ||
                                        ph->lastTraceStatus == IPS_OOB)) {
      ...
      if (ph->idepth_scaled < 0 || ph->residuals.size() == 0) {
        ...
        if (ph->isInlierNew()) {
          ...
          if (ph->idepth_hessian > setting_minIdepthH_marg &&
              ngoodRes >= setting_minInlierVotesForMarg) {
            flag_inin++;
...
void FullSystem::makeKeyFrame(FrameHessian *fh) {
...

  {
    ...
    coarseTracker_forNewKF->debugPlotIDepthMap(
        "id_predicted", &minIdJetVisTracker, &maxIdJetVisTracker,
        outputWrapper);
  }
...
void FullSystem::initializeFromInitializer(FrameHessian *newFrame) {
  ...
  // int numPointsTotal = pixelSelector->makeMaps(firstFrame->dIp,
  ...
  firstFrame->statistics_pointsActivatedForThisFrame =
      firstFrame->pointHessians.size();
