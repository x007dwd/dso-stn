...
bool RANSAC_Template<NUMTYPE>::execute(
    const Eigen::Matrix<NUMTYPE, Dynamic, Dynamic> &data,
    ...
    Eigen::Matrix<NUMTYPE, Dynamic, Dynamic> &out_best_model, bool verbose,
    const double p, const size_t maxIter) {

  assert(minimumSizeSamplesToFit >= 1);

  // Highly inspired on http://www.csse.uwa.edu.au/~pk/

  const size_t D = data.cols(); //  dimensionality
  const size_t Npts = data.rows();
  ...
  while (N > trialcount) {
    ...
    size_t count = 1;
    std::vector<Eigen::Matrix<NUMTYPE, Dynamic, Dynamic>> MODELS;

    while (degenerate) {
      // Generate s random indicies in the range 1..npts
      // ind.resize(minimumSizeSamplesToFit);

      // The +0.99... is due to the floor rounding afterwards when converting
      // from random double samples to size_t
      randomGenerator::drawUniformVector<int>(ind, minimumSizeSamplesToFit, 0.0,
                                              Npts - 1 + 0.999999);
      ...
      if (++count > maxDataTrials) {
        if (verbose)
          printf("[RANSAC] Unable to select a nondegenerate data set\n");
        break;
      }
    }
    ...
    bool update_estim_num_iters =
        (trialcount == 0); // Always update on the first iteration, regardless
                           // of the result (even for ninliers=0)
    ...
    if (update_estim_num_iters) {
      ...
      if (verbose)
        printf("[RANSAC] Iter #%u Estimated number of iters: %u  "
               "pNoOutliers = %f  #inliers: %u\n",
    ...
    if (verbose)
      printf("[RANSAC] trial %u out of %u \r", (unsigned int)trialcount,
  ...
  if (out_best_model.rows() > 0) { // We got a solution
    if (verbose)
...
template class dso::RANSAC_Template<float>;
template class dso::RANSAC_Template<double>;

#ifdef HAVE_LONG_DOUBLE
template class dso::RANSAC_Template<long double>;
#endif
}
